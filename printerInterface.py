...
#               print('new z offset:', new_offset)
        ...
        def getREST(self, path):
               # print("REST:", self.op.base_address + path)
                r = self.op.s.get(url=self.op.base_address + path, headers=self.op.s.headers)
        ...
        async def _postREST(self, path, json):
                print("postREST:", self.op.base_address + path, json)
                r = self.op.s.post(self.op.base_address + path, json=json)
                d = r.content.decode('utf-8')
                print("RESULT:", d)
        ...
        def init_Webservices(self):
                try:
                        ...
                        print('Base: ' + self.op.base_address)
                        resp = requests.get(url=self.op.base_address + '/api/printer', headers=self.op.s.headers)
                except ConnectionError:
                        print('REST API does not exist')
                        return
                else:
                        print('REST API exists')
                ...
                self.SHORT_BUILD_VERSION = self.getREST('/api/version')["api"]
...
#                self.Y_MAX_POS = int(volume[1])
        ...
        def GetFiles(self, refresh=False):
                if not self.files or refresh:
                        self.files = self.getREST('/api/files')["files"]
                names = []
                idx = 0
...
#                fan = data['fan']
                ...
                extruder = data['temperature']['tool0']
                Update = False
                try:
                        if self.thermalManager['temp_bed']['celsius'] != int(bed['actual']):
                                self.thermalManager['temp_bed']['celsius'] = int(bed['actual'])
                                Update = True
                        if self.thermalManager['temp_bed']['target'] != int(bed['target']):
                                self.thermalManager['temp_bed']['target'] = int(bed['target'])
                                Update = True
                        if self.thermalManager['temp_hotend'][0]['celsius'] != int(extruder['actual']):
                                self.thermalManager['temp_hotend'][0]['celsius'] = int(extruder['actual'])
                                Update = True
        ...
        def getPercent(self):
                if self.job_Info['state'] == "printing":
                        return self.job_Info['progress']['completion'] * 100
                else:
                        return 0

        def duration(self):
                if self.job_Info['state'] == "printing":
                        return self.job_Info['estimatedPrintTime']
                return 0
        ...
        def openAndPrintFile(self, filenum):
                ...
                self.postREST('/api/files/local/' + self.file_name, json={'command' : "select"})
                self.postREST('/api/job', json={'command': 'start'})
        ...
        def moveAbsolute(self, axis, position, speed):
                self.sendGCode('%s %s %s%s F%s%s' % ('G90', 'G1', axis, position, speed,
                        'G91'))
